...
public class AnimationSequenceDeserializer implements JsonDeserializer<AnimationSequence> {
    ...
    public AnimationSequence deserialize(JsonElement jsonElement, Type type, JsonDeserializationContext context) throws JsonParseException {
        ...
        String rawSkeleton = sequenceJsonObject.get(JOINT_SKELETON_KEY).getAsString();
        ResourceLocation parsedSkeleton = ResourceLocation.tryParse(rawSkeleton);
        if (parsedSkeleton == null) {
            throw new JsonParseException("Joint skeleton resource location " + rawSkeleton + " is invalid.");
        }
        ResourceLocation jointSkeletonLocation = parsedSkeleton.withPath("skeletons/" + parsedSkeleton.getPath() + ".json");
        ...
        if (sequenceJsonObject.has(TIME_MARKERS_KEY)) {
            ...
            timeMarkers.forEach((timeMarkerResourceLocation, timeMarkerElement) -> {
                JsonArray times = timeMarkerElement.getAsJsonArray();
                times.forEach(time -> sequenceBuilder.putTimeMarker(timeMarkerResourceLocation, TimeSpan.ofSeconds(time.getAsFloat())));
            });
        }
