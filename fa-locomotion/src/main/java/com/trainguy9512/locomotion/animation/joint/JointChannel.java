...
public final class JointChannel {
    ...
    public static JointChannel ofPartPose(PartPose partPose){
        return ofTranslationRotationScaleEuler(new Vector3f(partPose.x, partPose.y, partPose.z), new Vector3f(partPose.xRot, partPose.yRot, partPose.zRot), new Vector3f(1.0f, 1.0f, 1.0f), true);
    }
...
//                    case LOCAL -> this.transform.mulLocal(transform);
    ...
    public void transformPoseStack(PoseStack poseStack, float transformMultiplier) {
        ...
        com.trainguy9512.locomotion.util.PoseStackUtil.multiply(poseStack, matrix4f.setTranslation(this.getTranslation().div(new Vector3f(transformMultiplier))));
