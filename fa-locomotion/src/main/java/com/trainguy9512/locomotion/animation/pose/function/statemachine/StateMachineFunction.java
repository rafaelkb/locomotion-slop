...
public class StateMachineFunction extends TimeBasedPoseFunction<LocalSpacePose> {
    ...
    public @NotNull LocalSpacePose compute(PoseCalculationContext context) {
        ...
        for (String stateResourceLocation : uniqueStatesInLayerStack) {
            layerStackPoses.put(stateResourceLocation, this.states.get(stateResourceLocation).inputFunction.compute(context));
        }
    ...
    private void takeInitialStateIfResetting(PoseTickEvaluationContext context) {
        ...
        if (layerStackIsEmpty || (hasBecomeRelevant && this.resetsUponRelevant)) {
            ...
            String initialStateResourceLocation = this.initialStateFunction.apply(context);
            if (this.states.containsKey(initialStateResourceLocation)) {
                this.stateBlendLayerStack.addLast(new StateBlendLayer(
                        initialStateResourceLocation,
                        StateTransition.builder(initialStateResourceLocation)
                                .setTiming(Transition.INSTANT)
                                .isTakenIfTrue(transitionContext -> true)
                                .build())
                );
            } else {
                throw new IllegalStateException("Initial state " + initialStateResourceLocation + " not found to be present in the state machine");
            }
        }
    ...
    private Optional<StateTransition> testForOutboundTransition(PoseTickEvaluationContext context) {
        ...
        String currentActiveStateResourceLocation = this.stateBlendLayerStack.getLast().identifier;
        StateDefinition currentActiveStateDefinition = this.states.get(currentActiveStateResourceLocation);
        ...
        return currentActiveStateDefinition.outboundTransitions.stream()
                .filter(stateTransition -> {
                    ...
                    boolean targetIsNotCurrentActiveState = !Objects.equals(stateTransition.target(), currentActiveStateResourceLocation);
                    if(transitionTargetIncludedInThisMachine && targetIsNotCurrentActiveState){
                        StateTransitionContext transitionContext = new StateTransitionContext(
                                ...
                                this.states.get(currentActiveStateResourceLocation).inputFunction,
    ...
    private void takeTransition(PoseTickEvaluationContext context, StateTransition transition) {
        ...
        this.driversToUpdateOnStateChanged.forEach(driverKey -> {
            LOGGER.info(driverKey.getResourceLocation());
    ...
    private void tickPoseFunctionsInActiveStates(PoseTickEvaluationContext context, @Nullable String stateBeingEntered) {
        for (String stateResourceLocation : this.getStatesInLayerStack()) {
            StateDefinition stateDefinition = this.states.get(stateResourceLocation);
            PoseFunction<?> statePoseFunction = stateDefinition.inputFunction;
            boolean shouldResetStatePoseFunction = Objects.equals(stateBeingEntered, stateResourceLocation) && stateDefinition.resetUponEntry;
